...
start_old="function startAdd(id){const x=items.find(i=>i.id===id);"
start_new="function startAdd(id){if(String(id).startsWith('legacy-')){alert('Il menu di emergenza è solo consultazione. Riprova tra poco per inviare l’ordine.');return}const x=items.find(i=>i.id===id);"
